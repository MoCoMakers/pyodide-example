...
class DraggableDemoElement(PageElement):
    ...
    def getElementHTML(self):
        draggable_frame = '''
            <div id="'''+self.unique_id+'''" class="draggable" style="z-index: '''+str(self.z_index)+'''">
...
function dragElement(elmnt) {
    ...
    function dragMouseDown(e) {
        ...
        /*** A new addition in Demo 4 ***/
        updateZIndex(e.target.parentElement);
    }
...
# Another new script to add:
starter_js = '''
function updateZIndex(elem){
    console.log("Looking at z-index");
    console.log(elem);
    console.log(parseInt(elem.style.zIndex));
    console.log(max_z_index);
    if (parseInt(elem.style.zIndex) < max_z_index) {
        max_z_index = max_z_index + 1;
        elem.style.zIndex = max_z_index;
    }    
}
'''
ctx.add_header_script(starter_js)
...
#This section needs to be after all countable widgets are added
final_z_index_script = '''var max_z_index = '''+str(ctx.get_element_count()+10)+''';'''
ctx.add_footer_script(final_z_index_script)
